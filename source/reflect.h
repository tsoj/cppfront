...
#line 1207 "reflect.h2"
    {
        ...
        if (bitwise) {
            from_string += 
            "    result: " + cpp2::to_string(CPP2_UFCS(name)(t)) + " = (none);\n"
            "    ss: std::stringstream = (std::string(s));\n"
            "    word: std::string = ();\n"
            "    success := false;\n"
            "    found_first := false;\n"
            "    found_last := false;\n"
            ...
            for ( 
                  auto const& e : cpp2::move(enumerators) ) {
                from_string += 
            "        if \"" + cpp2::to_string(e.name) + "\" == word { \n"
            "            result |= " + cpp2::to_string(e.name) + ";\n"
            "            success = true;\n"
            "        }\n";
            }
